...
from transformers import AutoConfig, AutoModel, AutoModelForCausalLM, AutoTokenizer, LlamaTokenizer, LlamaTokenizerFast, HfArgumentParser, TrainingArguments, DataCollatorForSeq2Seq, set_seed
...
from models.llama_dlo.modeling_llama_dlo_exattn import LlamaDLOExAttnForCausalLM, LlamaDLOExAttnModel  # 🔍
...
MOD_CONFIGS = (LlamaDLOConfig, LlamaDLOExAttnConfig)
MOD_MODELS = (LlamaDLOModel, LlamaDLOExAttnModel, LlamaDLOForCausalLM, LlamaDLOExAttnForCausalLM)
...
class ModelArguments:
    ...
    dlo_capacity: Optional[float] = field(
        default=None
    )
    dlo_loss_coefficient: Optional[float] = field(
        default=None
    )
    dlo_loss_type: Optional[str] = field(
        default=None
    )
    feature_loss_coefficient: Optional[float] = field(
        default=None
    )
    rescale_hidden_states: Optional[bool] = field(
        default=None
    )
    scale_factor: Optional[float] = field(
        default=None
    )
    scale_gap: Optional[float] = field(
        default=None
    )


@dataclass
class DLOTrainingArguments(TrainingArguments):
    # 🔍 For DLO Training Strategy
    dlo_capacity_annealing_steps: Optional[int] = field(
        default=-1
    )

    # 🔍 For Dynamic Layer-Wise lr
...
def main():
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, DLOTrainingArguments))
    ...
    if isinstance(config, MOD_CONFIGS):
        if model_args.dlo_capacity is not None:
            config.dlo_capacity = model_args.dlo_capacity
            print(f"dlo_capacity: {model_args.dlo_capacity}")

        if model_args.dlo_loss_coefficient is not None:
            config.dlo_loss_coefficient = model_args.dlo_loss_coefficient
            print(f"dlo_loss_coefficient: {model_args.dlo_loss_coefficient}")

        if model_args.dlo_loss_type is not None:
            config.dlo_loss_type = model_args.dlo_loss_type
            print(f"dlo_loss_type: {model_args.dlo_loss_type}")

        if model_args.feature_loss_coefficient is not None:
            config.feature_loss_coefficient = model_args.feature_loss_coefficient
            print(f"feature_loss_coefficient: {model_args.feature_loss_coefficient}")

        if model_args.rescale_hidden_states is not None:
            config.rescale_hidden_states = model_args.rescale_hidden_states
            print(f"rescale_hidden_states: {model_args.rescale_hidden_states}")

        if model_args.scale_factor is not None:
            config.scale_factor = model_args.scale_factor
            print(f"scale_factor: {model_args.scale_factor}")

        if model_args.scale_gap is not None:
            config.scale_gap = model_args.scale_gap
            print(f"scale_gap: {model_args.scale_gap}")
    ###########################################################
    ...
    trainer = CustomizedTrainer(
        ...
        dlo_capacity_annealing_steps=training_args.dlo_capacity_annealing_steps,
        dynamic_lr=training_args.dynamic_lr,
    )
