...
class LlamaDLOConfig(PretrainedConfig):
    model_type = "llama_dlo"
    ...
    def __init__(
        ...
        # 🔍
        self.is_dlo = [False for _ in range(num_hidden_layers)] if is_dlo is None else is_dlo
        self.dlo_capacity = dlo_capacity
        self.dlo_loss_coefficient = dlo_loss_coefficient
        self.dlo_loss_type = dlo_loss_type
    ...
    def from_llama_config(
            config: LlamaConfig,
            is_dlo: list = None,
            dlo_capacity: float = 0.5,
            dlo_loss_coefficient: float = 0.1,
            dlo_loss_type: str = "self",
            rescale_hidden_states: bool = True,
